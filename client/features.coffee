...
Template.new_project.helpers
  creating_new_project: ->
    Session.get 'creating_new_project'

Template.new_project.events
  'click .new-project-link': (evt, tmpl) ->
    evt.preventDefault()
    # setting creating_new_project causes the template to render, exposing
    # the #new-project element.
    Session.set 'creating_new_project', true
...
Template.new_project.events okCancelEvents('#new-project',
  ok: (text, evt) ->
    ...
    evt.target.value = ""
    Session.set 'creating_new_project', false
  cancel: ->
    Session.set 'creating_new_project', false
)
...
Template.project_item.events
  'click .add': (evt, tmpl) ->
    Session.set 'new_feature_project_id', tmpl.data._id
  'click .destroy': ->
    Projects.remove @_id
